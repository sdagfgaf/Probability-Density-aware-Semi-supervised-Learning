...
class RatioMeter(AverageMeter):
    ...
    def update(self, scores, n=1):
        mask_bool = scores.ge(self.tao)
        val = mask_bool.float().mean()
        self.val = val
        self.sum += val * n
        self.count += n
        # self.avg = self.sum / (self.count + 1e-20)
...
class RatioMeter_right(AverageMeter):

    def __init__(self, tao: float):
        super().__init__()
        self.tao = tao

    def update(self, scores, mask_right, n=1):
        mask_bool = scores.ge(self.tao).float() * mask_right
        val = mask_bool.sum() / mask_right.sum()
        self.val = val
        self.sum += val * n
        self.count += n
        # self.avg = self.sum / (self.count + 1e-20)
