...
def prepare_args(rand_seed, config_file, args):
    description = "Train LASSL"
    config_path = config_file
    my_args = parse_commandline_args(args, name=description, filepath=config_path)
...
def _cal_lpa_label(source_feature, source_labels, target_feature, args, num_buffer_low=0):
    ...
    target_label_initial = torch.zeros(NumT, args.n_classes).cuda()  
    all_label = torch.cat((source_labels.float(), target_label_initial.float()), dim=0)  

    weight = torch.exp(torch.mm(all_feature, all_feature.t())/args.temperature) 
    values, indexes = torch.topk(weight, lp_knn_k, dim=1) 
    indexes = indexes.flatten() 
    points = all_feature[torch.arange(NumST).repeat_interleave(lp_knn_k),:]
    neighbour = all_feature[indexes,:] 
    midpoints = (points + neighbour)/2

    h = 5
    knn_mid_num = 45


    cos_similarity = torch.mm(midpoints, all_feature.t())
    _, mid_i = torch.topk(cos_similarity, knn_mid_num, dim=1)
    mid_knn = all_feature[mid_i.flatten(), :]
    midpoints = midpoints.repeat_interleave(knn_mid_num, dim = 0)
    distance = torch.sum((midpoints - mid_knn)**2, dim = 1)
    density = torch.exp(-distance/h)
    density = density.reshape(NumST*lp_knn_k, knn_mid_num)
    density = torch.sum(density, dim = 1)/knn_mid_num
    density = density/density.median().item()
    
    weight[weight < values[:, -1].view(-1, 1)] = 0 
    weight[torch.arange(NumST).repeat_interleave(lp_knn_k), indexes] *= density
...
def get_pseudo_label_propagation_wt_buffer(cur_features, cur_labels,
    ...
    if not args.use_buffer_bootstrap or args.bootstrap_nums == 1:
        ...
        mask = v_max.ge(args.bootstrap_thred) 
...
@torch.no_grad()
def get_sharpen_label(pseudo, var_temp):  
...
def train(epoch, model, ema_model, criteria_x, optim, lr_schdlr, dltrain_x, dltrain_u, 
            args, n_iters,logger, dist_class): # dl: data_loader
    ...
    for it in range(n_iters):
        ...
        # gain the raw output of model, excluding the final activate function.
        ...
        with torch.no_grad():
            ...
            # MDA some treatment to fulfill the MDA method
            if args.MDA:
            ...
            if mask_up_count == 500 and args.threshold<0.99:
                args.threshold += step
                mask_up_count = 0
                print('threshold up:', args.threshold)            
        ...
        if any(mask_bool):
            ...
            if args.CACL:
                ...
                # cont_prob: the prob_vectors of both labeled_data and unlabeled_data
...
def set_model(args):
    ...
    if args.checkpoint:
        ...
        # assert set(msg.missing_keys) == {"classifier.weight", "classifier.bias"},  msg.missing_keys
        assert len(msg.missing_keys) == 0,  msg.missing_keys
        print()
        print('loaded model from checkpoint: %s'%args.checkpoint)    
    model.train()
    model.cuda()
    if args.is_multigpu:
    ...
    if args.use_ema_model:
        ...
        if args.ema_checkpoint:
            ema_checkpoint = torch.load(args.ema_checkpoint)
            msg = model.load_state_dict(ema_checkpoint, strict=False)
            # assert set(msg.missing_keys) == {"classifier.weight", "classifier.bias"}
            assert len(msg.missing_keys) == 0,  msg.missing_keys
            print('loaded ema_model from checkpoint: %s'%args.checkpoint) 
            print()   
        else:
...
def main(my_args):
    ...
    for epoch in range(args.n_epoches):
        ...
        tmp_results = {'labeled_loss': loss_x,
                       ...
                       'cont_loss': loss_c,
                       "acc_num": acc_num,
                       'quantity': mask_mean, # the high confidence ratio?
...
if __name__ == '__main__':
    ...
    flag_run_once = True
    if flag_run_once:
        run_args = prepare_args(None, args.c, args)
        ...
        for each_seed in random_seeds:
            run_args = prepare_args(each_seed, args.c, args)
